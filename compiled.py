def testFunction(params):
	...
	for namespaces in ([None] if ((not params) or (not 'namespaces' in params)) else (params['namespaces'] if type(params['namespaces']) == list else [params['namespaces']])):
	...
	for Header in ([None] if ((not params) or (not 'Header' in params)) else (params['Header'] if type(params['Header']) == list else [params['Header']])):
		...
		for Security in ([None] if ((not Header) or (not 'Security' in Header)) else (Header['Security'] if type(Header['Security']) == list else [Header['Security']])):
			...
			for EncryptedKey in ([None] if ((not Security) or (not 'EncryptedKey' in Security)) else (Security['EncryptedKey'] if type(Security['EncryptedKey']) == list else [Security['EncryptedKey']])):
				...
				if 'IssuerSerial' in EncryptedKey:
	...
	for notification_text in ([None] if ((not params) or (not 'notification_text' in params)) else (params['notification_text'] if type(params['notification_text']) == list else [params['notification_text']])):
